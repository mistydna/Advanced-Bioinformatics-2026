...
conda install -y bwa samtools fastqc trimmomatic picard bedtools

# Install vcffilter (vcflib) and tabix via apt
# These were not reliably available through conda on this system
sudo apt install -y libvcflib-tools tabix
...
mkdir -p ~/ngs_pipeline/results/annotated/snpeff
wget -P ~/ngs_pipeline/results/annotated/snpeff/ \
    https://github.com/pcingola/SnpEff/releases/download/v5.2/snpEff_v5.2_core.zip
unzip ~/ngs_pipeline/results/annotated/snpeff/snpEff_v5.2_core.zip \
    -d ~/ngs_pipeline/results/annotated/snpeff/

# Install ANNOVAR (manual registration required at
# https://www.openbioinformatics.org/annovar/annovar_download_form.php)
# After downloading, upload to instance and extract:
mkdir -p ~/ngs_pipeline/results/annotated/annovar
tar -zxvf ~/ngs_pipeline/results/annotated/annovar/annovar.latest.tar.gz \
    -C ~/ngs_pipeline/results/annotated/annovar/

# Download ANNOVAR annotation databases
# refGene: gene-based annotation using RefSeq transcripts
perl ~/ngs_pipeline/results/annotated/annovar/annovar/annotate_variation.pl \
    -buildver hg19 -downdb -webfrom annovar refGene \
    ~/ngs_pipeline/results/annotated/annovar/annovar/humandb/
...
# Establish a consistent directory hierarchy for input data,
# intermediate files and final outputs.
# ------------------------------------------------------------

mkdir -p ~/ngs_pipeline/data/untrimmed_fastq \
         ~/ngs_pipeline/data/trimmed_fastq \
         ~/ngs_pipeline/data/reference \
         ~/ngs_pipeline/data/aligned_data \
         ~/ngs_pipeline/data/bed \
         ~/ngs_pipeline/results/fastqc/untrimmed \
         ~/ngs_pipeline/results/fastqc/trimmed \
         ~/ngs_pipeline/results/VCF \
         ~/ngs_pipeline/results/stats \
         ~/ngs_pipeline/results/annotated/annovar \
         ~/ngs_pipeline/results/annotated/snpeff \
         ~/ngs_pipeline/logs
...
wget -P ~/ngs_pipeline/data/untrimmed_fastq \
...
mv ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R1.fastq.qz \
   ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R1.fastq.gz
mv ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R2.fastq.qz \
   ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R2.fastq.gz

# Annotation BED file defines the targeted exome capture regions
wget -P ~/ngs_pipeline/data/bed \
    https://s3-eu-west-1.amazonaws.com/workshopdata2017/annotation.bed

# Download and decompress hg19 reference genome
wget -P ~/ngs_pipeline/data/reference \
    http://hgdownload.cse.ucsc.edu/goldenPath/hg19/bigZips/hg19.fa.gz
gunzip ~/ngs_pipeline/data/reference/hg19.fa.gz
...
fastqc -t 2 \
    ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R1.fastq.gz \
    ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R2.fastq.gz \
    -o ~/ngs_pipeline/results/fastqc/untrimmed/
...
trimmomatic PE \
    ...
    ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R1.fastq.gz \
    ~/ngs_pipeline/data/untrimmed_fastq/NGS0001.R2.fastq.gz \
    -baseout ~/ngs_pipeline/data/trimmed_fastq/NGS0001 \
...
fastqc -t 2 \
    ~/ngs_pipeline/data/trimmed_fastq/NGS0001_1P \
    ~/ngs_pipeline/data/trimmed_fastq/NGS0001_2P \
    -o ~/ngs_pipeline/results/fastqc/trimmed/

# ============================================================
# SECTION 2.3: ALIGNMENT AND POST-PROCESSING
# ============================================================

# Index reference genome
# samtools faidx creates .fai index required by FreeBayes
# bwa index creates FM-index files required for alignment
samtools faidx ~/ngs_pipeline/data/reference/hg19.fa
bwa index ~/ngs_pipeline/data/reference/hg19.fa

# Alignment with BWA-MEM
# Read group tags are required by Picard and FreeBayes downstream
# -I 250,50 specifies expected insert size mean and standard deviation
bwa mem \
    -t 4 \
    -v 1 \
    -R '@RG\tID:11V6WR1.111.D1375ACXX.1.NGS0001\tSM:NGS0001\tPL:ILLUMINA\tLB:nextera-ngs0001-blood\tPU:11V6WR1\tDT:2017-02-23' \
    -I 250,50 \
    ~/ngs_pipeline/data/reference/hg19.fa \
    ~/ngs_pipeline/data/trimmed_fastq/NGS0001_1P \
    ~/ngs_pipeline/data/trimmed_fastq/NGS0001_2P \
    > ~/ngs_pipeline/data/aligned_data/NGS0001.sam

# Convert SAM to BAM, sort by coordinate and index
# Coordinate sorting and indexing is required by all downstream tools
samtools view -h -b \
    ~/ngs_pipeline/data/aligned_data/NGS0001.sam \
    > ~/ngs_pipeline/data/aligned_data/NGS0001.bam

samtools sort \
    ~/ngs_pipeline/data/aligned_data/NGS0001.bam \
    > ~/ngs_pipeline/data/aligned_data/NGS0001_sorted.bam

samtools index ~/ngs_pipeline/data/aligned_data/NGS0001_sorted.bam
...
# rm ~/ngs_pipeline/data/aligned_data/NGS0001.sam
# rm ~/ngs_pipeline/data/aligned_data/NGS0001.bam

# Duplicate marking to reduce bias from PCR amplification
picard MarkDuplicates \
    I=~/ngs_pipeline/data/aligned_data/NGS0001_sorted.bam \
    O=~/ngs_pipeline/data/aligned_data/NGS0001_sorted_marked.bam \
    M=~/ngs_pipeline/results/stats/marked_dup_metrics.txt

samtools index ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_marked.bam
...
samtools view \
    -F 1796 \
    -q 20 \
    -o ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_marked.bam

samtools index ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam

# Alignment statistics
samtools flagstat \
    ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    > ~/ngs_pipeline/results/stats/NGS0001_flagstat.txt

samtools idxstats \
    ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    > ~/ngs_pipeline/results/stats/NGS0001_idxstats.txt

picard CollectInsertSizeMetrics \
    I=~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    O=~/ngs_pipeline/results/stats/NGS0001_insert_size_metrics.txt \
    H=~/ngs_pipeline/results/stats/NGS0001_insert_size_histogram.pdf \
    M=0.5
...
samtools depth \
    -a \
    ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    > ~/ngs_pipeline/results/stats/NGS0001_depth.txt
...
/usr/local/bin/freebayes \
    --bam ~/ngs_pipeline/data/aligned_data/NGS0001_sorted_filtered.bam \
    --fasta-reference ~/ngs_pipeline/data/reference/hg19.fa \
    --targets ~/ngs_pipeline/data/bed/annotation.bed \
    --vcf ~/ngs_pipeline/results/VCF/NGS0001.vcf

# Compress and index the raw VCF
bgzip ~/ngs_pipeline/results/VCF/NGS0001.vcf
tabix -p vcf ~/ngs_pipeline/results/VCF/NGS0001.vcf.gz
...
vcffilter -f "QUAL > 20 & QUAL / AO > 10 & SAF > 0 & SAR > 0 & RPR > 1 & RPL > 1" \
    ~/ngs_pipeline/results/VCF/NGS0001.vcf.gz \
    > ~/ngs_pipeline/results/VCF/NGS0001_filtered.vcf

bgzip ~/ngs_pipeline/results/VCF/NGS0001_filtered.vcf
tabix -p vcf ~/ngs_pipeline/results/VCF/NGS0001_filtered.vcf.gz
...
perl ~/ngs_pipeline/results/annotated/annovar/annovar/convert2annovar.pl \
    -format vcf4 \
    ~/ngs_pipeline/results/VCF/NGS0001_filtered.vcf.gz \
    > ~/ngs_pipeline/results/annotated/annovar/NGS0001_filtered.avinput
...
perl ~/ngs_pipeline/results/annotated/annovar/annovar/table_annovar.pl \
    ~/ngs_pipeline/results/annotated/annovar/NGS0001_filtered.avinput \
    ~/ngs_pipeline/results/annotated/annovar/annovar/humandb/ \
    -buildver hg19 \
    -out ~/ngs_pipeline/results/annotated/annovar/NGS0001_filtered \
    -remove \
...
java -jar ~/ngs_pipeline/results/annotated/snpeff/snpEff/snpEff.jar \
    -c ~/ngs_pipeline/results/annotated/snpeff/snpEff/snpEff.config \
    -v GRCh37.75 \
    ~/ngs_pipeline/results/VCF/NGS0001_filtered.vcf.gz \
    > ~/ngs_pipeline/results/annotated/snpeff/NGS0001_snpeff.ann.vcf
...
tabix -p vcf \
    ~/ngs_pipeline/results/annotated/snpeff/00-common_all.vcf.gz

# Cross-reference with dbSNP to add rsID annotations
# -Xmx8g allocates 8GB RAM for processing the large dbSNP file
java -Xmx8g \
    -jar ~/ngs_pipeline/results/annotated/snpeff/snpEff/SnpSift.jar annotate \
    -v ~/ngs_pipeline/results/annotated/snpeff/00-common_all.vcf.gz \
    ~/ngs_pipeline/results/annotated/snpeff/NGS0001_snpeff.ann.vcf \
    > ~/ngs_pipeline/results/annotated/snpeff/NGS0001_snpsift_dbsnp.ann.vcf
...
java -jar ~/ngs_pipeline/results/annotated/snpeff/snpEff/SnpSift.jar filter \
    ...
    ~/ngs_pipeline/results/annotated/snpeff/NGS0001_snpsift_dbsnp.ann.vcf \
    > ~/ngs_pipeline/results/annotated/snpeff/NGS0001_exonic.ann.vcf

# Step 2: remove variants present in dbSNP
java -jar ~/ngs_pipeline/results/annotated/snpeff/snpEff/SnpSift.jar filter \
    -n "(ID =~ 'rs')" \
    ~/ngs_pipeline/results/annotated/snpeff/NGS0001_exonic.ann.vcf \
    > ~/ngs_pipeline/results/annotated/snpeff/NGS0001_exonic_novel.ann.vcf
